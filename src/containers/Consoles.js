...
class Consoles extends Component {
  ...
  componentDidMount() {
    if(this.state.catalog == undefined){
      axios.get(`http://localhost:3001/products/list/all/${this.state.consoleName}/1/release_date/-1`) // 특정 catalog에 대해 발매일에 대해 내림차순
        .then((res) => {
        ...
        axios.get(`http://localhost:3001/products/list/all/${this.state.consoleName}/1/total_sell/-1`) // 특정 platform에 대해 판매량에 대해 내림차순
        .then((res) => {
          console.log(res.data);
          var datas = res.data.slice(0, 4);
